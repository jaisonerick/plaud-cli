...
- `speaker identify` opens a page that plays the stretches where each unnamed voice is speaking and takes the name, offering the people already known as you type. It also shows what the service already hears: who each voice is closest to and by how much, and which of the unnamed voices are one voice, so naming one offers the name to the rest rather than saving it for them. That costs a single request and no audio, because the embeddings are already stored and the whole of it is arithmetic over them. Each name is registered the moment it is typed rather than at the end, so a tab closed halfway leaves the voices already named named; when the page is finished the transcripts are rewritten. `internal/identify` holds the scan, the server and the page. `PLAUD_NO_BROWSER` stops it opening a window, for a run driven from somewhere else.
...
- `speaker outside <recording-id> <key> --reason "…"` says the meeting did not hold a voice at all, which is what a recording catches every time somebody walks in or talks near it. `--reason` is required: what tells a voice left out for being somebody at the next table from one left out by mistake is the reason, and in six months neither the date nor the label says.
...
**A name the store contradicts is refused.** Naming a voice teaches it, and a voice learned under the wrong name goes on claiming that person in every transcription anybody makes afterwards. So the refusal needs both halves to say so: the voice is nowhere near the person it was given to, and it is confidently somebody else. A voice matching nobody is one nobody has taught yet and is taken as typed, because refusing there would leave a real person unnameable until somebody deleted a row. Whoever was in the room outranks the measurement, and `--despite-timbre` is how they say so.

**A voice the meeting did not hold is a verdict, not a deletion.** Editing those turns out of the file by hand does not survive, because the next rendering of that recording writes them again; the verdict lives with the voices, keyed by voice id, so every `fetch` and every `sync` from then on leaves the turns out and drops the entry from the `voices:` block, which is what stops it being counted as pending. Naming the voice takes the verdict back. Transcribing a recording again does not carry it, because that separates the voices afresh and the ones ruled out no longer exist.
